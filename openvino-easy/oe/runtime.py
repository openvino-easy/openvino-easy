...
import numpy as np
from typing import Dict, Any, Union, Optional, List, Callable, Awaitable
...
class RuntimeWrapper:
    ...
    def _init_tokenizer(self):
        ...
        try:
                ...
                if local_path.exists():
                    try:
                        ...
                        if self._is_likely_text_model():
                            logging.warning(f"⚠️  Failed to load tokenizer from {local_path}: {e}")
                            logging.info("💡 Consider installing transformers for better text processing: pip install transformers")
            ...
            if self._is_likely_text_model():
                ...
                logging.info("💡 For better text processing, install transformers: pip install transformers")
        ...
        if self.tokenizer is not None:
            try:
                ...
                if hasattr(self.tokenizer, 'model_max_length') and self.tokenizer.model_max_length < 1000000:
                    return min(self.tokenizer.model_max_length, 2048)  # Cap at 2048 for performance
            except:
                pass
    ...
    def _preprocess_input(self, input_data: Union[str, np.ndarray, Dict[str, Any]]) -> Dict[str, np.ndarray]:
        ...
        if isinstance(input_data, str) and len(input_data) < 1000:  # Only cache reasonably sized strings
            ...
            with self._cache_lock:
                ...
                if cache_key in self._preprocessing_cache:
                    self._cache_hits += 1
                    return self._preprocessing_cache[cache_key].copy()  # Return copy to avoid mutation
    ...
    def _tokenize_fallback(self, text: str) -> Dict[str, np.ndarray]:
        ...
        for input_name, input_info in self.input_info.items():
            ...
            if len(expected_shape) == 2:  # [batch_size, seq_len]
                max_length = expected_shape[1] if expected_shape[1] > 0 else self._get_optimal_sequence_length()
                
                # Pad or truncate
                if len(tokens) > max_length:
                    tokens = tokens[:max_length-1] + [tokens[-1]]  # Keep last token (often EOS)
                else:
                ...
                if len(expected_shape) == 3:  # Maybe embeddings? [batch, seq, dim]
                    seq_len = expected_shape[1] if expected_shape[1] > 0 else min(len(tokens), 128)
                    # Create simple embeddings (normalized random vectors)
                    embeddings = np.random.normal(0, 0.1, expected_shape).astype(np.float32)
                    input_tensor = embeddings
                elif len(expected_shape) == 4:  # Likely vision model
                    input_tensor = np.random.normal(0.45, 0.25, expected_shape).astype(np.float32)
                    input_tensor = np.clip(input_tensor, 0, 1)
                else:
                    # Unknown shape, create random data
                    input_tensor = np.random.normal(0, 0.1, expected_shape).astype(np.float32)
    ...
    def _basic_tokenize(self, text: str) -> List[int]:
        ...
        common_vocab = {
            ...
            'in': 8, 'on': 9, 'at': 10, 'to': 11, 'of': 12, 'for': 13,
            'with': 14, 'by': 15, 'from': 16, 'about': 17, 'into': 18,
            'through': 19, 'during': 20, 'before': 21, 'after': 22,
            'is': 23, 'are': 24, 'was': 25, 'were': 26, 'be': 27,
            'been': 28, 'being': 29, 'have': 30, 'has': 31, 'had': 32,
            'do': 33, 'does': 34, 'did': 35, 'will': 36, 'would': 37,
            'could': 38, 'should': 39, 'may': 40, 'might': 41, 'must': 42,
            'can': 43, 'not': 44, 'no': 45, 'yes': 46, 'all': 47,
            'this': 48, 'that': 49, 'these': 50, 'those': 51,
            'i': 52, 'you': 53, 'he': 54, 'she': 55, 'it': 56,
            'we': 57, 'they': 58, 'me': 59, 'him': 60, 'her': 61,
            'us': 62, 'them': 63, 'my': 64, 'your': 65, 'his': 66,
            'our': 67, 'their': 68, 'what': 69, 'where': 70, 'when': 71,
            'why': 72, 'how': 73, 'who': 74, 'which': 75,
        ...
        if token_ids:
            ...
            token_ids = [common_vocab['<cls>'], common_vocab['<pad>'], common_vocab['<sep>']]
    ...
    def _is_likely_text_model(self) -> bool:
        ...
        source_path = self.model_info.get('source_path', '').lower()
        text_indicators = ['bert', 'gpt', 'roberta', 'distilbert', 'albert', 'bloom', 'opt', 't5']
    ...
    async def infer_async(self, input_data: Union[str, np.ndarray, Dict[str, Any]], **kwargs) -> Any:
        """
    ...
    def _run_single_inference(self, processed_input: Dict[str, np.ndarray]) -> Any:
        ...
        try:
            ...
            for output_node in self.compiled_model.outputs:
                output_name = output_node.get_any_name()
                output_data[output_name] = infer_request.get_output_tensor(output_node.index).data.copy()
    ...
    def get_pool_stats(self) -> Dict[str, Any]:
        ...
        with self._request_pool_lock:
            return {
                ...
                "active_requests": len(self.async_infer_requests) - len(self._available_requests),
                "device": self.device,
    ...
    def _safe_infer(self, input_data: Union[str, np.ndarray, Dict[str, Any]], **kwargs) -> Any:
    ...
    def is_loaded(self) -> bool:
        ...
        return (hasattr(self, 'compiled_model') and 
                self.compiled_model is not None and
                hasattr(self, 'infer_request') and 
                self.infer_request is not None)
    ...
    def _is_audio_file(self, file_path: Union[str, Path]) -> bool:
        ...
        audio_extensions = {'.wav', '.mp3', '.flac', '.ogg', '.m4a', '.aac', '.wma', '.mp4', '.avi'}
        return path.suffix.lower() in audio_extensions
    
    def _preprocess_audio_input(self, audio_path: Union[str, Path]) -> Dict[str, np.ndarray]:
            ...
            try:
                with wave.open(str(audio_path), 'rb') as wav_file:
                    ...
                    if sample_rate != 16000:
                        logging.warning(f"⚠️  Audio sample rate is {sample_rate}Hz, expected 16kHz. "
