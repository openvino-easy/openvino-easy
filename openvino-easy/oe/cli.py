...
def cmd_doctor(args):
    ...
    for device in all_devices:
        ...
        try:
            ...
            if device.startswith("GPU") and "NVIDIA" in device_name.upper():
                print(f"    └─ [WARNING] NVIDIA GPU detected - install Intel GPU drivers for Intel GPU support")
    ...
    if functional_count == 0:
        print("[WARNING] No functional devices detected - OpenVINO installation may be corrupted")
    elif functional_count < total_count:
        print("[WARNING] Some devices need attention - run 'oe doctor --fix <device>' for help")
    else:
...
def list_devices(args):
    ...
    print("Device Status:")
    for device in all_devices:
        status = "[OK] Functional" if device in validated_devices else "[FAIL] Not functional"
        print(f"  {device}: {status}")
...
def cmd_npu_doctor(args):
    ...
    print(f"NPU in available devices: {'[OK] Yes' if npu_status['npu_in_available_devices'] else '[FAIL] No'}")
    print(f"NPU functional: {'[OK] Yes' if npu_status['npu_functional'] else '[FAIL] No'}")
    print(f"Driver status: {npu_status['driver_status']}")
    ...
    if npu_gen != "unknown":
        ...
        if capabilities:
            ...
            if capabilities.get("expected_stable_diffusion_fps"):
                print(f"Expected Stable Diffusion performance: ~{capabilities['expected_stable_diffusion_fps']:.1f} img/s")
            if capabilities.get("expected_dialog_gpt_tps"):
                print(f"Expected DialoGPT performance: ~{capabilities['expected_dialog_gpt_tps']} tokens/s")
...
def run_benchmark(args):
    ...
    try:
        ...
        # Run benchmark
        if not args.json:
            print_info(f"Benchmarking (warmup: {args.warmup_runs}, runs: {args.benchmark_runs})...")
        stats = oe.benchmark(
        ...
        if args.json:
            ...
            if COLORS_AVAILABLE:
                ...
                print(f"Average Latency: {Fore.GREEN}{stats['mean_ms']:.2f} ms{Style.RESET_ALL}")
                print(f"Throughput: {Fore.GREEN}{stats['fps']:.1f} FPS{Style.RESET_ALL}")
            else:
        ...
        if args.json:
            ...
            if not models:
                ...
                for model in models:
                    ...
                    total_size += size_mb
                    if COLORS_AVAILABLE:
                        print(f"  {Fore.CYAN}{model['name']}{Style.RESET_ALL}: {Fore.GREEN}{size_mb:.1f} MB{Style.RESET_ALL}")
                    else:
...
def cache_size(args):
    ...
    try:
        ...
        if args.json:
            ...
            print(f"Models: {cache_info['models_size_mb']:.1f} MB ({cache_info['model_count']} models)")
    ...
    try:
        if args.force:
            ...
            if not matching_model:
                ...
                if not args.json:
                    ...
                    result = {"removed": False, "error": "Use --force flag to confirm deletion in JSON mode"}
        ...
        if args.json:
            ...
            if result.get('removed'):
                ...
                print_error(f"Failed to remove model: {result.get('error', 'Unknown error')}")
    ...
    try:
        if args.models and not args.force:
            ...
            if not args.json:
                ...
                print(f"This will delete ALL {cache_info['model_count']} cached models ({cache_info['models_size_mb']:.1f} MB)")
                ...
                result = {"error": "Use --force flag to confirm dangerous operations in JSON mode"}
        else:
        ...
        if args.json:
            ...
            if result.get('temp_cache_cleared'):
                print_success(f"Temp cache cleared: {result['temp_size_freed_mb']:.1f} MB freed")
            if result.get('models_cleared'):
                print_success(f"Models cleared: {result['models_removed']} models, {result['models_size_freed_mb']:.1f} MB freed")
    ...
    try:
        ...
        if args.json:
            ...
            if not results:
                ...
                print_header(f"Search Results for '{args.query}' ({len(results)} found)")
                for model in results:
                    ...
                    if COLORS_AVAILABLE:
                        ...
                        print(f"    Type: {pipeline} | Downloads: {downloads:,} | Likes: {likes}")
                    else:
                        print(f"  {model['id']}")
                        print(f"    Type: {pipeline} | Downloads: {downloads:,} | Likes: {likes}")
                    ...
                    if model.get('gated', False):
                        print(f"    [GATED MODEL - requires approval]")
                    print()
...
def model_validate(args):
    ...
    try:
        ...
        if args.json:
            ...
            if results['failed'] > 0:
                print_warning("Some models failed validation. Consider reinstalling them.")
...
def model_benchmark(args):
    ...
    try:
        if not args.json:
            ...
            print_warning("This may take several minutes depending on the number of models")
        ...
        if args.json:
            ...
            print_header(f"Benchmark Results ({results['benchmarked']}/{results['total_models']} models)")
            ...
            if summary['fastest_model']:
                print_success(f"Fastest: {summary['fastest_model']['id']} ({summary['fastest_model']['fps']:.1f} FPS on {summary['fastest_model']['device']})")
            if summary['slowest_model']:
                print_info(f"Slowest: {summary['slowest_model']['id']} ({summary['slowest_model']['fps']:.1f} FPS on {summary['slowest_model']['device']})")
            ...
            for result in results['results']:
                if 'error' in result:
                    ...
                    print(f"    Device: {bench.get('device', 'unknown')} | FPS: {bench.get('fps', 0):.1f} | Latency: {bench.get('mean_ms', 0):.1f}ms")
...
def create_parser():
    ...
    cache_list_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    cache_list_parser.set_defaults(func=cache_list)
    
    # cache size
    cache_size_parser = cache_subparsers.add_parser("size", help="Show cache size information")
    cache_size_parser.add_argument("--cache-dir", help="Custom cache directory")
    cache_size_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    cache_size_parser.set_defaults(func=cache_size)
    
    # cache remove
    cache_remove_parser = cache_subparsers.add_parser("remove", help="Remove a specific model")
    ...
    cache_remove_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")
    cache_remove_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    cache_remove_parser.set_defaults(func=cache_remove)
    
    # cache clear
    cache_clear_parser = cache_subparsers.add_parser("clear", help="Clear cache (temp files only by default)")
    cache_clear_parser.add_argument("--cache-dir", help="Custom cache directory")
    cache_clear_parser.add_argument("--models", action="store_true", help="WARNING DANGEROUS: Also clear all cached models")
    cache_clear_parser.add_argument("--force", action="store_true", help="Skip safety confirmations (VERY DANGEROUS)")
    cache_clear_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    ...
    models_search_parser.add_argument("query", help="Search query (model name, description, etc.)")
    models_search_parser.add_argument("--limit", type=int, default=10, help="Maximum number of results (default: 10)")
    models_search_parser.add_argument("--type", choices=["text", "image", "audio", "vision"], help="Filter by model type")
    models_search_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    models_search_parser.set_defaults(func=model_search)
    
    # models info
    models_info_parser = models_subparsers.add_parser("info", help="Get detailed information about a model")
    ...
    models_info_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    models_info_parser.set_defaults(func=model_info)
    
    # models install
    models_install_parser = models_subparsers.add_parser("install", help="Download and convert a model")
    models_install_parser.add_argument("model", help="Model ID to install")
    models_install_parser.add_argument("--dtype", choices=["fp16", "int8", "fp16-nf4"], default="fp16", help="Model precision")
    models_install_parser.add_argument("--cache-dir", help="Custom cache directory")
    models_install_parser.add_argument("--force", action="store_true", help="Force reinstall if model exists")
    models_install_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    models_install_parser.set_defaults(func=model_install)
    
    # models validate
    models_validate_parser = models_subparsers.add_parser("validate", help="Validate model integrity")
    models_validate_parser.add_argument("--model", help="Specific model to validate (validates all if not specified)")
    models_validate_parser.add_argument("--cache-dir", help="Custom cache directory")
    models_validate_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    models_validate_parser.set_defaults(func=model_validate)
    
    # models benchmark
    models_benchmark_parser = models_subparsers.add_parser("benchmark", help="Benchmark all installed models")
    models_benchmark_parser.add_argument("--cache-dir", help="Custom cache directory")
    models_benchmark_parser.add_argument("--warmup", type=int, default=3, help="Warmup runs per model")
    models_benchmark_parser.add_argument("--runs", type=int, default=10, help="Benchmark runs per model")
    models_benchmark_parser.add_argument("--output", help="Save results to JSON file")
    models_benchmark_parser.add_argument("--json", action="store_true", help="Output in JSON format")
