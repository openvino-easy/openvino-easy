...
def _generate_dummy_input(compiled_model, batch_size=1):
    ...
    for input_node in compiled_model.inputs:
        ...
        shape = [batch_size if (dim == -1 or (i == 0 and dim == 1)) else dim for i, dim in enumerate(shape)]
        # Generate random data
        input_data[input_node.get_any_name()] = np.random.randn(*shape).astype(np.float32)
    return input_data

def _generate_realistic_inputs(pipeline, config: BenchmarkConfig) -> List[Dict[str, np.ndarray]]:
    """Generate realistic input data based on model type."""
    inputs = []
    
    # Check if it's a text model
    if hasattr(pipeline.runtime, 'tokenizer') and pipeline.runtime.tokenizer is not None:
        ...
        for text in text_samples[:config.benchmark_runs]:
            try:
                ...
                inputs.append(_generate_dummy_input(pipeline.compiled_model, config.batch_size))
    ...
    # Check if it's a vision model  
        ...
        for i in range(config.benchmark_runs):
            if config.image_sizes:
                ...
                img_data = np.random.rand(config.batch_size, 3, 224, 224).astype(np.float32)
...
def _measure_system_resources(func, *args, **kwargs):
    ...
    try:
        ...
        return result, {
            ...
            "cpu_usage_percent": (cpu_after - cpu_before) / duration if duration > 0 else 0
        }
    except ImportError:
        # psutil not available
        return func(*args, **kwargs), {"memory_usage_mb": None, "cpu_usage_percent": None}

def benchmark_model_enhanced(compiled_model, 
                           config: BenchmarkConfig,
                           device_name: Optional[str] = None,
                           model_name: str = "unknown") -> BenchmarkResult:
    """
    ...
    throughput_fps = round((config.batch_size * 1000) / mean_ms, 1) if mean_ms > 0 else 0
...
def benchmark_model_multithreaded(compiled_model,
                                 config: BenchmarkConfig,
                                 device_name: Optional[str] = None,
                                 model_name: str = "unknown") -> BenchmarkResult:
    """
    ...
    infer_requests = [compiled_model.create_infer_request() for _ in range(config.max_threads)]
...
def save_benchmark_results(results: Union[Dict[str, Any], BenchmarkResult], 
...
def _add_npu_performance_analysis(result: BenchmarkResult, pipeline) -> None:
    ...
    """
    try:
        from ._core import check_npu_driver, get_npu_generation
        ...
        if model_type == "stable_diffusion":
            ...
            result.npu_analysis["expected_performance"]["stable_diffusion_fps"] = expected_fps
            result.npu_analysis["performance_comparison"]["vs_expected_fps"] = {
                ...
                "performance_level": _categorize_performance_vs_expected(actual_fps, expected_fps)
            }
            ...
            result.npu_analysis["expected_performance"]["text_generation_tps"] = expected_tps
            result.npu_analysis["performance_comparison"]["vs_expected_tps"] = {
                ...
                "performance_level": _categorize_performance_vs_expected(actual_tps, expected_tps)
            }
...
def _detect_benchmark_model_type(pipeline) -> str:
    ...
    try:
        ...
        elif any(model in source_path for model in ["gpt", "dialog", "t5", "bert", "text"]):
            return "text_generation"
        elif any(model in source_path for model in ["resnet", "efficientnet", "vit", "image"]):
...
def compare_benchmarks(results_list: List[Union[Dict[str, Any], BenchmarkResult]], 
    ...
    comparison = {
        "models": model_names or [r.get("model_name", f"model_{i}") for i, r in enumerate(results_dicts)],
    ...
    if len(results_dicts) > 1:
        ...
        comparison["relative_throughput"] = [tp / best_throughput if best_throughput > 0 else 0 for tp in comparison["throughput_fps"]]
        
        # Performance analysis
        comparison["analysis"] = {
            "fastest_model": comparison["models"][comparison["fps"].index(best_fps)],
            "highest_throughput": comparison["models"][comparison["throughput_fps"].index(best_throughput)],
            "most_consistent": comparison["models"][comparison["p99_latency_ms"].index(min(comparison["p99_latency_ms"]))],
            "performance_spread": {
...
async def benchmark_pipeline_async(pipeline,
