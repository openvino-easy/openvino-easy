...
def _download_with_retry(repo_id: str, 
                        cache_dir: Path,
                        max_retries: int = 3,
    ...
    for attempt in range(max_retries):
        try:
            logging.info(f"📥 Downloading {repo_id} (attempt {attempt + 1}/{max_retries})")
            
            # Add retry parameters
            local_model_path = snapshot_download(
                repo_id=repo_id,
                cache_dir=cache_dir / "hf_cache",
                allow_patterns=["*.safetensors", "*.bin", "*.json", "*.txt", "*.md", "*.py", "*.onnx"],
            ...
            if not local_path.exists():
                raise ModelNotFoundError(f"Downloaded path does not exist: {local_path}")
            
            # Basic integrity check
            if not any(local_path.iterdir()):
                raise CorruptedModelError(f"Downloaded model directory is empty: {local_path}")
            ...
            if "not found" in str(e).lower() or "404" in str(e):
                raise ModelNotFoundError(f"Model '{repo_id}' not found on Hugging Face Hub: {e}")
            elif "network" in str(e).lower() or "connection" in str(e).lower() or "timeout" in str(e).lower():
                if attempt < max_retries - 1:
                    wait_time = retry_delay * (2 ** attempt)  # Exponential backoff
                    logging.warning(f"⚠️  Network error, retrying in {wait_time:.1f}s: {e}")
                    time.sleep(wait_time)
                    continue
                else:
                    raise NetworkError(f"Failed to download '{repo_id}' after {max_retries} attempts: {e}")
            elif "permission" in str(e).lower() or "forbidden" in str(e).lower():
                raise ModelNotFoundError(f"Access denied for model '{repo_id}'. Model may be private or require authentication: {e}")
            else:
                # Unknown error - retry if we have attempts left
                if attempt < max_retries - 1:
                    wait_time = retry_delay * (2 ** attempt)
                    logging.warning(f"⚠️  Download failed, retrying in {wait_time:.1f}s: {e}")
                    time.sleep(wait_time)
                    continue
                else:
                    break
    ...
    if last_exception:
        raise NetworkError(f"Failed to download '{repo_id}' after {max_retries} attempts. Last error: {last_exception}")
    else:
...
def _safe_model_conversion(convert_func, model_path: str, *args, **kwargs):
    ...
    try:
        ...
        raise ModelConversionError(
            f"Insufficient memory for model conversion. "
            f"Try reducing batch size or converting on a machine with more RAM."
        ) from e
    except Exception as e:
    ...
    try:
        ...
        for i, input_node in enumerate(model.inputs):
            if not hasattr(input_node, 'shape'):
                raise CorruptedModelError(f"Input {i} missing shape information: {model_path}")
        
        for i, output_node in enumerate(model.outputs):
            if not hasattr(output_node, 'shape'):
                raise CorruptedModelError(f"Output {i} missing shape information: {model_path}")
        
        logging.info(f"✅ Model integrity verified: {len(model.inputs)} inputs, {len(model.outputs)} outputs")
...
def _detect_model_type(model_path: Path) -> str:
    ...
    if (model_path / "config.json").exists():
        try:
            ...
            optimum_compatible = [
                "bert", "roberta", "distilbert", "albert", "electra", "deberta", "camembert",
                "gpt2", "gpt_neo", "gpt_neox", "gpt_j", "codegen", "opt", "bloom", "llama",
                "t5", "mt5", "bart", "pegasus", "marian", "blenderbot", "whisper", "wav2vec2"
            ]
            
            # Check for audio models specifically
            audio_indicators = ["whisper", "wav2vec2", "wavlm", "hubert", "speecht5", "bark", "vall-e"]
            if (model_type in audio_indicators or 
                any(arch.lower().startswith(tuple(audio_indicators)) for arch in architectures) or
                any(audio_arch in model_type for audio_arch in audio_indicators) or
                task in ["automatic-speech-recognition", "text-to-speech", "audio-classification"]):
                return "transformers_audio"
            
            # Check if it's a known optimum-compatible model
            if (model_type in optimum_compatible or 
                any(arch.lower().startswith(tuple(optimum_compatible)) for arch in architectures) or
                any(opt_arch in model_type for opt_arch in optimum_compatible)):
                return "transformers_optimum"
            
            # Check for vision transformers and other vision models
            vision_indicators = ["vit", "deit", "swin", "beit", "convnext", "resnet", "efficientnet"]
            if (any(indicator in model_type for indicator in vision_indicators) or
                any(any(indicator in arch.lower() for indicator in vision_indicators) for arch in architectures)):
                return "transformers_vision"
            
            # Check for multimodal models
            multimodal_indicators = ["clip", "blip", "llava", "flamingo", "kosmos"]
            if (any(indicator in model_type for indicator in multimodal_indicators) or
                any(any(indicator in arch.lower() for indicator in multimodal_indicators) for arch in architectures)):
            ...
            has_tokenizer = any((model_path / f).exists() for f in [
                "tokenizer.json", "tokenizer_config.json", "vocab.txt", "vocab.json", 
                "merges.txt", "special_tokens_map.json"
            ])
            
            if has_tokenizer:
                return "transformers_optimum"  # Likely text model that works with optimum
    ...
    if onnx_files:
        ...
        try:
            ...
            if (model_path / "tokenizer.json").exists():
                return "onnx_text"
            elif any((model_path / f).exists() for f in ["labels.txt", "imagenet_classes.txt"]):
                return "onnx_vision"
            else:
                return "onnx"
        except:
    ...
    pytorch_files = (list(model_path.glob("*.pt")) + 
    ...
    tf_files = (list(model_path.glob("*.pb")) + 
...
def _convert_diffusers_with_optimum(model_path: str, output_dir: str, dtype: str = "fp16"):
...
def _convert_transformers_with_optimum(model_path: str, output_dir: str, model_id: str, dtype: str = "fp16"):
    """Convert transformers model using optimum-intel."""
    try:
        from optimum.intel import OVModelForCausalLM, OVModelForSequenceClassification, OVModelForQuestionAnswering
        ...
        elif any(arch.lower().startswith(('gpt', 'bloom', 'opt')) for arch in getattr(config, 'architectures', [])):
            # Causal LM
...
def load_model(model_id_or_path: str, dtype: str = "fp16", cache_dir: str | Path = "~/.cache/oe"):
    ...
    try:
        ...
        local_model_path = _download_with_retry(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                if model_type == "diffusers":
                ...
                elif model_type in ["transformers_optimum", "transformers_vision", "transformers_multimodal", "transformers_audio"]:
                    # Use optimum-intel for known compatible transformers models
                    logging.info(f"🔄 Converting {model_type} model with optimum-intel...")
                    model_xml_path = _safe_model_conversion(
                    ...
                    model = _safe_model_conversion(
                        _convert_with_direct_ov,
                        str(local_path), "onnx", dtype
                    )
                    
                elif model_type in ["tensorflow"]:
                    # TensorFlow models - try direct conversion
                    logging.info(f"🔄 Converting TensorFlow model...")
                    ...
                    model = _safe_model_conversion(
                        _convert_tensorflow_model,
                        str(local_path), dtype
                    )
                    
                elif model_type in ["transformers_direct", "pytorch_transformers", "safetensors_transformers"]:
                    # Try direct OpenVINO conversion first, fallback to optimum-intel
                    logging.info(f"⚠️  Attempting direct conversion for {model_type} model...")
                    try:
                        model = ov.convert_model(
                            str(local_path),
                            compress_to_fp16=(dtype == "fp16")
                    ...
                    try:
                        ...
                        model_name = model_id_or_path.split('/')[-1] if '/' in model_id_or_path else model_id_or_path
                        raise ModelConversionError(
                    ...
                    logging.info(f"⚠️  Unknown model format. Analyzing structure and trying conversion strategies...")
                    
                    # Provide detailed analysis of what was found
                    files_found = []
                    for pattern in ["*.json", "*.bin", "*.pt", "*.pth", "*.onnx", "*.pb", "*.h5", "*.safetensors"]:
                        files = list(Path(local_path).glob(pattern))
                        if files:
                            files_found.extend([f.name for f in files[:3]])  # Limit to first 3
                    
                    logging.info(f"📁 Files found: {', '.join(files_found[:10])}")  # Show first 10
                    ...
                    try:
                        ...
                        if (Path(local_path) / "config.json").exists():
                            try:
                                logging.info("🔄 Strategy 2: Optimum-intel conversion (config.json found)...")
                            ...
                            conversion_attempts.append("Optimum-intel: No config.json found")
        ...
        try:
            ...
            raise ModelLoadError(f"Failed to cache model '{model_id_or_path}': {e}") from e
    ...
    except (ModelLoadError, ModelNotFoundError, ModelConversionError, 
            NetworkError, UnsupportedModelError, CorruptedModelError):
        # Re-raise our custom exceptions
        raise
    except Exception as e:
        # Catch any other unexpected errors
        raise ModelLoadError(f"Unexpected error loading model '{model_id_or_path}': {e}") from e 