...
from .loader import (
    load_model, 
    ModelLoadError, ModelNotFoundError, ModelConversionError, 
...
from ._core import detect_device, get_available_devices, check_npu_driver, get_npu_generation, get_npu_capabilities
...
def _validate_cache_path(original_path: str) -> None:
    ...
    try:
        ...
        dangerous_resolved = ['windows', 'system32', 'program files', '/bin', '/sbin', '/usr']
        for dangerous in dangerous_resolved:
            if dangerous in resolved_str and len(resolved_str.split(os.sep)) <= 4:  # Not too deep
                raise ValueError(f"Resolved path points to system directory: {resolved_path}")
...
class Pipeline:
    ...
    async def infer_batch_async(self, input_batch, max_concurrent=None, **kwargs):
        ...
        Returns:
            List of model outputs in the same order as inputs
        """
        return await self.runtime.infer_batch_async(input_batch, max_concurrent, **kwargs)
...
def load(model_id_or_path: str, 
    ...
    if device is None:
        if fallback_device in available_devices:
            ...
            logging.warning(f"Preferred devices {attempted_devices} not available, using fallback: {device}")
        else:
            device = "CPU"  # Ultimate fallback
            logging.warning(f"No preferred devices available, using CPU")
    ...
    if dtype == "fp16-nf4":
        if device == "NPU":
            ...
            if npu_status.get("npu_functional") and npu_status.get("capabilities", {}).get("supports_fp16_nf4", False):
                logging.info(f"FP16-NF4 precision supported on {npu_status.get('npu_generation', 'unknown')} NPU")
            else:
                warnings.warn(
        ...
        try:
            ...
            if attempt_device == device and compile_config:
                try:
                    ...
                    logging.warning(f"Succeeded with default config, some optimizations disabled")
                    break
...
def benchmark(warmup_runs: int = 5, benchmark_runs: int = 20, **kwargs) -> Dict[str, Any]:
    """
...
class _ModelsNamespace:
    ...
    @staticmethod
    def search(query: str, limit: int = 10, model_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if model_type:
                ...
                type_mapping = {
                    'text': ['text-generation', 'text2text-generation', 'conversational'],
                    'image': ['text-to-image', 'image-classification', 'object-detection'],
    ...
    def info(model_id: str, cache_dir: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            elif any(tag in info['remote_info']['tags'] for tag in ['llm', 'text-generation']):
    ...
    @staticmethod
    def install(model_id: str, dtype: str = "fp16", cache_dir: Optional[str] = None, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            for installed_model in installed_models:
                if model_id in installed_model['name'] and dtype in installed_model['name']:
                    return {
                        'installed': True,
    ...
    @staticmethod
    def validate(model_name: str = None, cache_dir: Optional[str] = None) -> Dict[str, Any]:
        ...
        for model in models_to_check:
            ...
            try:
                ...
                if not model_path.exists():
                    ...
                    if not bin_files:
                        model_result['warnings'].append("No .bin files found (may be embedded)")
    ...
    @staticmethod
    def benchmark_all(cache_dir: Optional[str] = None, warmup: int = 3, runs: int = 10) -> Dict[str, Any]:
        ...
        for model in models:
            try:
                ...
                # Load and benchmark
                oe.load(model_id, dtype=dtype, cache_dir=cache_dir)
                bench_result = oe.benchmark(warmup_runs=warmup, benchmark_runs=runs)
                oe.unload()
        ...
        if results['benchmarked'] > 0:
            results['summary']['average_fps'] = round(total_fps / results['benchmarked'], 2)
    ...
    @staticmethod
    def remove(model_name: str, cache_dir: Optional[str] = None, confirm: bool = True) -> Dict[str, Any]:
            ...
            if confirm:
                return {
                    ...
                    "error": f"SAFETY CHECK: This will delete {model_count} models ({size_mb} MB, {file_count} files).\n" +
                            ...
                            f"To proceed: oe.models.clear(confirm=False)\n" +
                            f"Consider using oe.models.remove('specific-model') instead."
                }
...
class _CacheNamespace:
    ...
    def size(cache_dir: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_count": len(list(models_path.iterdir())) if models_path.exists() else 0
        }
    
    @staticmethod
    def clear(cache_dir: Optional[str] = None, models: bool = False, confirm: bool = None) -> Dict[str, Any]:
