...
def get_npu_capabilities(npu_generation: str) -> dict:
    ...
    capabilities = {
        "lunar_lake": {
            ...
            "generation": "3rd",
            "features": ["Advanced quantization", "FP16-NF4 mixed precision", "Enhanced efficiency"]
        },
        "arrow_lake": {
            "supports_fp16": True,
            ...
            "generation": "3rd",
            "features": ["Desktop optimization", "FP16-NF4 mixed precision", "High performance"]
        },
        "core_ultra": {
            "supports_fp16": True,
...
def check_npu_driver() -> dict:
    ...
    if not result["npu_in_available_devices"]:
        ...
        result["recommendations"].append("Install Intel NPU driver from intel.com/content/www/us/en/support")
        result["recommendations"].append("Check if NPU is enabled in BIOS/UEFI settings")
        result["recommendations"].append("Verify you have a supported Intel processor (Core Ultra series)")
        return result
    ...
    try:
        ...
        if not device_name:
            ...
            try:
                supported_props = core.get_property("NPU", "SUPPORTED_PROPERTIES")
                ...
                if npu_generation in ["lunar_lake", "arrow_lake"]:
                    result["recommendations"].append(f"🎉 Latest generation NPU detected: {npu_generation}")
                    result["recommendations"].append("Consider using FP16-NF4 precision for optimal performance")
                    result["recommendations"].append(f"Expected Stable Diffusion performance: ~{result['capabilities']['expected_stable_diffusion_fps']:.1f} img/s")
                elif npu_generation == "core_ultra":
                    result["recommendations"].append("Core Ultra NPU detected - good performance expected")
                elif npu_generation == "meteor_lake":
                    result["recommendations"].append("First-generation NPU - consider upgrading for better performance")
                
            except Exception as e:
                result["driver_status"] = "driver_incomplete"
                result["recommendations"].append("NPU driver may be incomplete - reinstall latest driver")
