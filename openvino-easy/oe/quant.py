...
def _generate_smart_calibration_data(model, model_info: Optional[Dict[str, Any]] = None, num_samples: int = 100):
    ...
    if model_info and model_info.get('source_path'):
        ...
        if calibration_data:
            logging.info(f"✅ Generated {len(calibration_data)} calibration samples using real dataset")
    ...
    if calibration_data:
        logging.info(f"⚠️  Generated {len(calibration_data)} calibration samples using typed heuristics")
        return calibration_data
    
    # Fallback to random data with warning
    logging.warning("⚠️  Using random calibration data - this may reduce quantization accuracy")
    warnings.warn(
...
def _is_text_model(model, model_info: Dict[str, Any]) -> bool:
    ...
    text_indicators = ['bert', 'gpt', 'dialogpt', 'roberta', 'distilbert', 'albert', 'bloom', 'opt']
...
def _generate_text_calibration_data(model, model_info: Dict[str, Any], num_samples: int):
    ...
    try:
        ...
        if real_texts:
            ...
            logging.info("📝 Using curated text samples (consider installing 'datasets' for better accuracy)")
            diverse_samples = [
                # News/factual
            ...
            text_samples = (diverse_samples * (num_samples // len(diverse_samples) + 1))[:num_samples]
    ...
    try:
        ...
        for input_node in model.inputs:
            ...
            if real_images and len(fixed_shape) == 4:
                logging.info(f"🖼️  Using real image dataset with {len(real_images)} samples")
                ...
                for i in range(min(num_samples, len(real_images))):
                    ...
                    if fixed_shape[1] == 3:  # CHW format [B, C, H, W]
                        if real_img.shape != (3, fixed_shape[2], fixed_shape[3]):
                            ...
                            sample_data = np.transpose(img_array, (2, 0, 1))  # HWC -> CHW
                        else:
                        ...
                        sample_data = np.random.normal(0.45, 0.25, fixed_shape).astype(np.float32)
                ...
                logging.info("🎨 Using synthetic image data with ImageNet statistics (consider installing 'datasets' for better accuracy)")
                ...
                for i in range(num_samples):
                    ...
                    if len(fixed_shape) == 4:  # [B, C, H, W] or [B, H, W, C]
                        ...
                        sample_data = np.random.normal(0, 0.25, fixed_shape).astype(np.float32)
...
def _try_real_text_dataset(model_info: Dict[str, Any], num_samples: int) -> Optional[List[str]]:
    ...
    try:
        ...
        if any(indicator in source_path for indicator in ['gpt', 'bloom', 'opt', 'llama']):
        ...
        elif any(indicator in source_path for indicator in ['bert', 'roberta', 'distilbert']):
        ...
        logging.info("💡 Install 'datasets' for better calibration: pip install datasets")
    except Exception as e:
...
def _try_real_vision_dataset(num_samples: int) -> Optional[List[np.ndarray]]:
    ...
    try:
        ...
        logging.info("💡 Install 'datasets' for better vision calibration: pip install datasets pillow")
    except Exception as e:
...
def quantize_model(model: 'ov.Model', dtype: str = "int8", cache_dir: Union[str, Path] = "~/.cache/oe", 
                   model_info: Optional[Dict[str, Any]] = None, 
                   calibration_data: Optional[List[Dict[str, np.ndarray]]] = None,
                   quantization_method: str = "auto", **kwargs) -> 'ov.Model':
    """
    ...
    if not NNCF_AVAILABLE:
        warnings.warn("NNCF not available. Install with 'pip install openvino-easy[quant]' for INT8 quantization support.")
        return model
    ...
    if quantization_method == "auto":
        ...
        quantization_method = "full_quantization" if calibration_data or model_info else "weights_only"
    ...
    try:
        if quantization_method == "weights_only":
            ...
            quantized_model = quantize(
                ...
                subset_size=min(len(calibration_data), quant_config["stat_subset_size"]),
        ...
        logging.info(f"✅ Model quantized using {quantization_method} with {len(calibration_data) if calibration_data else 0} calibration samples")
...
def get_quantization_stats(model: 'ov.Model', quantized_model: 'ov.Model') -> Dict[str, Any]:
    """
