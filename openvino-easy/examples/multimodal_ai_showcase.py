                ...
                try:
                    ...
                    for description in text_descriptions:
                        try:
                            ...
                            img_result = oe.infer(img_path)
                            
                            # Process text (NEW API - simplified for demo)
                            text_result = oe.infer(description)
...
def demo_multimodal_search():
    ...
    database = [
        {"image": demo_images[0], "text": "outdoor scene with people", "category": "lifestyle"},
        {"image": demo_images[1], "text": "animal portrait photography", "category": "nature"},
    ] if len(demo_images := download_demo_images()) >= 2 else []
    
    # Add some generated text descriptions
    database.extend([
        {"text": "modern architecture and urban design", "category": "architecture"},
    ...
    try:
        ...
        for i, item in enumerate(database):
            try:
                ...
                print(f"  ✅ Processed item {i+1}: {item.get('text', 'image')[:50]}...")
        ...
        for query in search_queries:
            ...
            try:
                ...
                query_result = oe.infer(query)
                query_embedding = np.random.randn(512)  # Placeholder
                ...
                for i, result in enumerate(similarities[:3]):
                    item = result["item"]
                    display_text = item.get("text", f"Image: {Path(item.get('image', '')).name}")
                    print(f"    {i+1}. {display_text[:60]}... "
...
def demo_creative_ai_applications():
    ...
    try:
        ...
        print(f"🔍 Extracted visual elements:")
        for element in visual_elements:
            print(f"  - {element}")
        
        # Step 2: Generate image prompts
        image_prompts = [
            f"magical forest with ancient whispering trees, mystical atmosphere",
            f"crystal cave glowing with ethereal blue light, fantasy art",
            f"young explorer in magical forest, adventure scene"
        ]
    ...
    for modality in modalities:
        ...
        print(f"{modality:<12} {data['latency']:<12.2f} {data['accuracy']:<10.2f} {data['memory']:<12}")
