...
class ModelRegistry:
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    logger.info(f"Model {name} loaded successfully in {load_time:.2f}s on {info['device']}")
                    return True
                    
                except Exception as main_error:
                    logger.warning(f"Failed to load main model {config.model_id}: {main_error}")
                    ...
                    if config.fallback_models:
                        for fallback_id in config.fallback_models:
                            try:
                                ...
                                logger.info(f"Fallback model {fallback_id} loaded in {load_time:.2f}s")
                                return True
    ...
    def infer(self, name: str, input_data: Any, timeout: Optional[float] = None) -> InferenceResult:
        ...
        try:
            ...
            if self._current_model != name:
                if not self.load_model(name):
                    ...
                    health_status['details']['error'] = 'Failed to load model for health check'
                    return health_status
...
class ProductionModelService:
                ...
                for name in self.registry.loaded_models.keys():
                    try:
                        ...
                        if health['status'] in ['error', 'slow']:
                            logger.warning(f"Health issue detected for {name}: {health}")
                    except Exception as e:
    ...
    def batch_infer(self, model_name: str, input_batch: List[Any], 
...
def demo_production_setup():
    ...
    print(f"Text generation result: Success={text_result.success}, "
    ...
    avg_time = sum(r.inference_time for r in batch_results if r.success) / max(successful_results, 1)
    ...
    for model_name, stats in metrics['models'].items():
        ...
        print(f"  Requests: {stats['total_requests']} (success rate: {stats.get('success_rate', 0):.1%})")
