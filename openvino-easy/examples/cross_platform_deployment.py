...
class PlatformOptimizer:
    ...
    def _get_threading_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'thread_affinity': self.os_name == 'Linux'  # Linux supports thread affinity
        }
    ...
    def _get_memory_config(self) -> Dict[str, Any]:
        ...
        return {
            'max_memory_gb': min(available_memory * memory_fraction, total_memory * 0.8),
    ...
    def _get_platform_specific_config(self) -> Dict[str, Any]:
        ...
        if self.os_name == 'Windows':
            ...
            config.update({
                'use_accelerate_framework': self.platform_info.get('apple_silicon', False),
                'optimize_for_apple_silicon': self.platform_info.get('apple_silicon', False)
            })
...
class DeploymentManager:
    ...
    def create_optimized_pipeline(self, model_name: str, model_type: str = "general") -> oe.Pipeline:
        """Create an optimized pipeline for the current platform."""
        print(f"Creating optimized pipeline for {self.platform_info['os']} {self.platform_info['architecture']}")
    ...
    def generate_dockerfile(self, model_name: str, base_image: str = "python:3.11-slim") -> str:
...
'''
    ...
    def generate_docker_compose(self, model_name: str, services: List[str] = None) -> str:
...
'''
    ...
    def generate_kubernetes_manifests(self, model_name: str, replicas: int = 2) -> Dict[str, str]:
...
def demonstrate_platform_detection():
    ...
    print(f"  CPU Cores: {platform_info['physical_cores']} physical, {platform_info['cpu_count']} logical")
    print(f"  Memory: {platform_info['memory_gb']:.1f}GB total, {platform_info['available_memory_gb']:.1f}GB available")
...
def demonstrate_optimized_inference():
    ...
    try:
        ...
        pipeline = manager.create_optimized_pipeline(model_name, model_type="text")
...
def demonstrate_environment_detection():
    ...
    detected_cloud = [provider for provider, detected in cloud_providers.items() if detected]
