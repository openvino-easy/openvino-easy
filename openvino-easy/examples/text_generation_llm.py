    ...
    for model_id in models_to_try:
        ...
        try:
            ...
            for prompt in prompts:
                ...
                print(f"Generated text ({inference_time:.2f}s): {generated_text[:200]}...")
            
            # Execute performance benchmarking
            print("\nPerformance analysis in progress...")
            benchmark_results = oe.benchmark()
            print(f"Average latency: {benchmark_results.get('avg_latency_ms', 'N/A')}ms")
            print(f"Throughput: {benchmark_results.get('throughput_fps', 'N/A')} inferences/sec")
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if isinstance(inference_result, dict) and 'generated_text' in inference_result:
                    ai_response = inference_result['generated_text']
                elif isinstance(inference_result, list) and len(inference_result) > 0:
                    ai_response = inference_result[0].get('generated_text', str(inference_result[0]))
                else:
...
def demo_performance_comparison():
    ...
    evaluation_prompt = "The future of technology is"
    ...
    for config in precision_configurations:
        for device in available_devices:
            ...
            try:
                ...
                print(f"Load: {load_time:.2f}s, Latency: {result['avg_latency']:.1f}ms, "
    ...
    if benchmark_results:
        ...
        print(f"Best Latency: {best_latency['config']} "
