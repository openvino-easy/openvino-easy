...
def create_test_audio_file(filepath: str, duration: float = 3.0, frequency: float = 440.0, 
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for model_id in whisper_models:
            ...
            try:
                ...
                stats = oe.benchmark(
                    input_data=test_files[0],
                    warmup_runs=2,
                    benchmark_runs=5
                )
                
                print(f"  Average latency: {stats.get('avg_latency_ms', 0):.0f}ms")
                print(f"  Throughput: {stats.get('throughput_fps', 0):.2f} inferences/sec")
                ...
                try:
                    ...
                    if isinstance(audio_data, np.ndarray):
                        ...
                        if i == 0:  # Save first sample
                            output_file = f"/tmp/tts_sample_{model_id.split('/')[-1]}.wav"
                            try:
                                # Normalize and convert to int16
                                audio_normalized = audio_data / np.max(np.abs(audio_data)) * 0.8
...
def demo_real_time_audio_processing():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"🔄 Processing {num_chunks} audio chunks of {chunk_duration}s each...")
            ...
            for chunk_idx in range(num_chunks):
                ...
                create_test_audio_file(str(chunk_file), duration=chunk_duration, 
                                     frequency=frequency)
                ...
                print(f"  Chunk {chunk_idx+1}: {processing_time:.3f}s -> '{transcription[:30]}...'")
                
                # Check if processing is fast enough for real-time
                if processing_time > chunk_duration:
                    print(f"    ⚠️ Slower than real-time ({processing_time:.2f}s > {chunk_duration}s)")
        ...
        print(f"  Min/Max processing time: {min_processing_time:.3f}s / {max_processing_time:.3f}s")
        print(f"  Real-time factor: {real_time_factor:.2f}x")
        
        if real_time_factor < 1.0:
            print(f"  ✅ Real-time capable: {1/real_time_factor:.1f}x faster than real-time")
        else:
