...
def main():
    ...
    parser.add_argument(
        "--mode", 
        choices=["fast", "full", "unit", "integration", "e2e", "coverage", "performance", "audio"],
        default="fast",
    ...
    if args.mode == "fast":
        ...
        cmd = base_cmd + ["tests/test_audio.py", "tests/test_e2e_real_models.py::TestE2ERealModels::test_audio_model_e2e"]
        ...
        # Run with coverage
        cmd = base_cmd + ["--cov=oe", "--cov-report=html", "--cov-report=term", "tests/", "-m", "not slow"]
