...
from oe.quant import quantize_model, _generate_random_calibration_data, _get_quant_cache_key, get_quantization_stats
...
@patch('oe.quant._get_model_checksum', return_value="mock_checksum")  # Mock checksum function
@patch('oe.quant.compress_weights')
@patch('oe.quant.ov.Core')
@patch('oe.quant.ov.save_model')
@patch('oe.quant.Path')
def test_quantize_model_int8_new(mock_path, mock_save, mock_core, mock_compress, mock_checksum, mock_nncf):
    ...
    with patch('oe.quant.ov.__version__', '2025.2.0'):
        result = quantize_model(mock_model, dtype="int8", quantization_method="weights_only")
...
def test_get_quantization_stats():
    ...
    assert "NNCF" in stats["quantization_method"] or stats["quantization_method"] == "None" 