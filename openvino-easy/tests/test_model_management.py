...
import oe
from oe import _ModelsNamespace
...
class TestModelInfo:
    ...
    def test_info_remote_only(self, mock_hf_info):
        ...
        with patch.object(oe.models, 'list', return_value=[]):
            ...
            assert info['local'] == False
            assert info['remote'] == True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(oe.models, 'list') as mock_list:
                ...
                with patch('openvino.Core') as mock_core:
                    ...
                    assert results['models'][0]['valid'] == True
    ...
    def test_validate_invalid_model(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(oe.models, 'list') as mock_list:
                ...
                assert results['models'][0]['valid'] == False
                assert any('No .xml files found' in error for error in results['models'][0]['errors'])
...
class TestModelBenchmarkAll:
    ...
    def test_benchmark_all_success(self, mock_unload, mock_benchmark, mock_load):
        ...
        with patch.object(oe.models, 'list') as mock_list:
            mock_list.return_value = [
                {
                    'name': 'test--model1--fp16--hash1',
                    'size_mb': 100.0
                },
                {
                    'name': 'test--model2--int8--hash2', 
                    'size_mb': 50.0
                }
            ]
