...
def test_compare_benchmarks():
    ...
    comparison = compare_benchmarks([results1, results2, results3], ["model1", "model2", "model3"])
...
def test_benchmark_model_fps_calculation(mock_time, mock_generate_input):
    ...
    mock_model = MagicMock()
    mock_model.inputs = [MagicMock()]
    mock_generate_input.return_value = {"input": np.random.randn(1, 3, 224, 224).astype(np.float32)}
