...
def test_load_hf_model_new(mock_path, mock_core, mock_save, mock_convert, mock_download):
    ...
    mock_path.side_effect = lambda x: mock_path_instance if x == "test/model" else mock_local_path
    ...
    assert mock_download.call_count >= 0  # May or may not be called depending on cache logic
