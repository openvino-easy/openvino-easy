...
class TestPerformanceRegression:
    ...
    def create_dummy_model_for_benchmark(self, temp_dir: Path, model_type: str = "text"):
        ...
        if model_type == "text":
            ...
            input_node = ov.opset11.parameter(input_shape, dtype=np.float32, name="input_ids")
            
            # Simple embedding-like operation
            weight = ov.opset11.constant(np.random.randn(128, 768).astype(np.float32))
            output = ov.opset11.matmul(input_node, weight, transpose_a=False, transpose_b=False)
            ...
            # Simple convolution
            weight = ov.opset11.constant(np.random.randn(64, 3, 3, 3).astype(np.float32))
            conv = ov.opset11.convolution(input_node, weight, strides=[1, 1], pads_begin=[1, 1], pads_end=[1, 1])
            output = ov.opset11.relu(conv)
            
        elif model_type == "audio":
            # Simple 1D convolution for audio-like processing
            input_shape = [1, 16000]  # [batch, audio_samples]
            input_node = ov.opset11.parameter(input_shape, dtype=np.float32, name="input_features")
            ...
            conv = ov.opset11.convolution(reshaped, weight, strides=[128], pads_begin=[128], pads_end=[128])
            output = ov.opset11.relu(conv)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result['status'] == 'regression_detected':
                ...
                for regression in result['regressions']:
                    print(f"  - {regression['metric']}: {regression['percent_change']:.1f}% slower")
                    print(f"    Baseline: {regression['baseline']:.2f}, Current: {regression['current']:.2f}")
            
            if result.get('improvements'):
                print("✅ Performance improvements:")
                for improvement in result['improvements']:
                    print(f"  - {improvement['metric']}: {improvement['percent_change']:.1f}% better")
    ...
    def test_cache_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            start_time = time.time()
            oe.load(str(model_path), device_preference=["CPU"], cache_dir=temp_path / "cache")
            ...
            cache_speedup = first_load_time / second_load_time if second_load_time > 0 else 1.0
            ...
            assert cache_speedup >= 0.8, f"Cache should not slow down loading (speedup: {cache_speedup:.2f}x)"