...
class TestAudioSupport:
    ...
    def test_audio_file_detection(self):
        ...
        for test_file in test_files:
            with patch.object(Path, 'exists', return_value=True):
                assert runtime._is_audio_file(test_file), f"Should detect {test_file} as audio"
        ...
        for test_file in non_audio_files:
            with patch.object(Path, 'exists', return_value=True):
                assert not runtime._is_audio_file(test_file), f"Should not detect {test_file} as audio"
    ...
    def test_audio_file_path_handling(self):
        ...
        try:
            ...
            with patch('librosa.load') as mock_librosa:
                mock_librosa.return_value = (np.random.randn(16000).astype(np.float32), 16000)
    ...
    @pytest.mark.skipif(LIBROSA_AVAILABLE, reason="librosa is available, skip fallback test")
